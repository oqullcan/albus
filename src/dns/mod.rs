...
pub(crate) fn secure_query_id() -> u16 {
    ...
    let nanos = std::time::SystemTime::now()
        ...
        .unwrap_or(0x5678);
    let fallback = (nanos ^ std::process::id().wrapping_mul(0x9E37)) as u16;
    if fallback == 0 {
        1
    } else {
        fallback
...
mod tests {
    ...
    #[test]
    fn test_secure_query_id_distributes() {
        // same statistical gate for the shared run-4 helper
        let ids: Vec<u16> = (0..512).map(|_| secure_query_id()).collect();
        let uniq: std::collections::HashSet<u16> = ids.iter().cloned().collect();
        assert!(
            uniq.len() > 480,
            "shared query IDs must distribute, unique={}",
            uniq.len()
        );
    }
}
